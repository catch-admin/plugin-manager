        ...
        <div v-loading="loading" class="min-h-[400px] mt-5">
            ...
            <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 items-stretch">
                <el-card v-for="plugin in plugins" :key="plugin.id" shadow="never" class="plugin-card overflow-hidden h-full">
                    ...
                    <!-- 关键词标签 - 固定高度保持卡片一致 -->
                    <div v-if="plugin.keywords && plugin.keywords.length > 0" class="flex gap-x-2">
                        <el-tag
                    ...
                    <template #footer>
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex justify-between gap-2">
                                <!-- 已安装：更新 + 卸载按钮 -->
                                <div v-if="plugin.is_installed">
                                    <!-- 更新按钮（有新版本时显示） -->
                                    <div v-if="hasUpdate(plugin)">
                                        <el-select
                                    ...
                                    <span v-else class="inline-flex items-center px-2 py-1 rounded-full bg-green-50 text-green-600 dark:bg-green-900/30 dark:text-green-400 text-xs">
                                      <Icon name="check-circle" class="w-3.5 h-3.5 mr-1" />
                                      已安装 v{{ plugin.installed_version }}
                                    </span>
                                </div>
                                <!-- 未安装：版本选择 + 安装按钮 -->
                                <div class="flex gap-x-2" v-if="!plugin.is_installed && plugin.versions && plugin.versions.length > 0">
                                    <el-select
                                        v-model="plugin.selected_version"
                                        size="small"
                                        style="width: 80px"
