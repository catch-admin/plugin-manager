<template>
    <div class="p-4 bg-white dark:bg-regal-dark">
        <el-tabs v-model="activeTab" class="bg-white dark:bg-regal-dark p-4 rounded-lg">
            <!-- 本地插件
...
const refreshMenus = async () => {
    try {
        ...
        // 强制刷新菜单并获取新的异步路由
        const asyncRoutes = permissionsStore.getAsyncMenusFrom(toRaw(userStore.getPermissions), true)
        // 动态添加新路由
        ...
        // 刷新菜单显示
        permissionsStore.getMenusFrom(userStore.getPermissions, true)
    } catch (error) {
...
// 操作成功回调
const handleProgressSuccess = async (plugin: Plugin) => {
    const msgMap = {
        install: '插件安装成功',
        update: '插件更新成功',
        uninstall: '插件卸载成功'
