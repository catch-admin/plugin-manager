...
class Plugin
{
     ...
     * 执行模块数据库迁移.
     */
    public static function migrateModule(string $moduleName): void
    {
        self::migrate(CatchAdmin::getModuleMigrationPath($moduleName));
    }
     ...
     * @return void
     */
    public static function deleteAdminModuleMenu(string $module, string|array|null $permissionMark = null): void
    {
        DeleteModuleMenusEvent::dispatch($module, $permissionMark);
    }
    ...
    public static function allRoutes(): array
    {
        ...
        foreach ($plugins as $plugin) {
            ...
            $routesPath = $pluginPath . DIRECTORY_SEPARATOR . 'routes';
            if (! Str::of($routesPath)->start(base_path())) {
                $routesPath = base_path($routesPath);
            }
    ...
    public static function all(): array
    {
        ...
        foreach ($composerFiles as $composerFile) {
            ...
            if ($composerData && ! empty($composerData['name'])) {
                $plugins[$composerData['name']]['path'] = dirname($composerFile);
            }
        }
     ...
     * @throws FileNotFoundException
     */
    public static function migrationPath(string $pluginName): string
    {
    ...
    public static function createMenu(string $name, string $frontRoute,
        ...
        return array_merge([
            ...
            'icon' => $icon,
            'module' => $module,
            'permission_mark' => $controller ? $controller . ($controllerMethod ? '@' . $controllerMethod : '') : '',
            'component' => $component,
            'type' => $type,
